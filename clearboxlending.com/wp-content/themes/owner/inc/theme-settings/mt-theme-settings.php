...
if ( ! class_exists( 'Owner_Settings' ) ) :
	...
	?>
			<div id="mt-theme-message" class="updated notice owner-message">
				...
				<div class="submit">
					<button class="mt-get-started button button-primary button-hero" data-done="<?php esc_html_e( 'Done!', 'owner' ); ?>" data-process="<?php esc_html_e( 'Processing', 'owner' ); ?>" data-redirect="<?php echo esc_url( wp_nonce_url( add_query_arg( 'owner-hide-notice', 'welcome', admin_url( 'themes.php' ).'?page=owner-settings&tab=demos' ) , 'owner_hide_notices_nonce', '_owner_notice_nonce' ) ); ?>">
	...
	<?php
		...
		public function demos_screen() {
			...
			if ( empty( $demodata ) || $demodata == false ) {
				$owner_library = new Owner_Demo_Library();
				$demodata = $owner_library->retrieve_demo_by_activatetheme();
				if ( $demodata ) {
